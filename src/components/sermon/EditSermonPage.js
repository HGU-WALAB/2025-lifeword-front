...
const EditSermonPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const cleanContent = formData.contentText.replace(
      ...
      const updatedSermon = {
        ...
        worshipType:
          formData.worshipType === "기타"
            ? formData.customWorshipType
            : formData.worshipType,
  ...
  return (
    <Container>
      ...
      <AutoSaveStatus visible={!!autoSaveStatus}>
        {autoSaveStatus}
      </AutoSaveStatus>
    </Container>
  );
...
const FormContainer = styled.form`
  ...
  grid-template-columns: ${(props) =>
    props.isMetaOpen ? "400px 1fr" : "50px 1fr"};
  gap: 32px;
  max-width: 1200px;
  margin: 0 auto;
  margin-bottom: 40px;
