        ...
        if (response.success && response.data.length > 0) {
          const uniqueBooks = response.data.filter(
            (book, index, self) =>
              index ===
              self.findIndex(
                (b) => b.book === book.book && b.long_label === book.long_label
              )
          );
      ...
      try {
        ...
        if (response.success) {
          const uniqueChapters = [
            ...new Set(response.data.map((verse) => verse.chapter)),
          ];
          setChapters(uniqueChapters);

          const currentChapterVerses = response.data.filter(
  ...
  return (
    <Container>
      <TestamentSelector>
        <TestamentButton
          active={testament === "구"}
          onClick={() => setTestament("구")}
        >
          구약
        </TestamentButton>
        <TestamentButton
          active={testament === "신"}
          onClick={() => setTestament("신")}
        >
          신약
        </TestamentButton>
